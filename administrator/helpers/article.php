...
use Joomla\Utilities\ArrayHelper;
		...
		if (!$cid) : $cid = Factory::getApplication()->input->get('cid', array(), 'array'); endif;
...
$data['alias'] 			= OutputFilter::stringURLSafe($alias);
...
// Lets store it!    
$row                        =            Table::getInstance('Content', 'Table');
$row->bind($data);
$row->check();
			// Store the row.
			if (!$row->store())
			{
			Factory::getApplication()->enqueueMessage(Text::_($this->text_prefix . ' '), 'error');
				return false;
			}
//print_r ($row);exit;
