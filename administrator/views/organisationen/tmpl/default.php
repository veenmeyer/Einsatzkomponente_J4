...
if ($saveOrder)
{
	...
	$version = new JVersion;
	if ($version->isCompatible('3.0')) :
	JHtml::_('sortablelist.sortable', 'organisationList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
	endif;
...
<?php endif;?>
		...
		<table class="table table-striped" id="organisationList">
			<thead>
				...
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_EINSATZKOMPONENTE_ORGANISATIONEN_LINK', 'a.link', $listDirn, $listOrder); ?>
				</th>
                
                <?php if ($params->get('gmap_action','0')) : ?>
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_EINSATZKOMPONENTE_ORGANISATIONEN_GMAP_LATITUDE', 'a.gmap_latitude', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_EINSATZKOMPONENTE_ORGANISATIONEN_GMAP_LONGITUDE', 'a.gmap_longitude', $listDirn, $listOrder); ?>
				</th>
 			    <?php  endif; ?>
                
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_EINSATZKOMPONENTE_ORGANISATIONEN_FFW', 'a.ffw', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_EINSATZKOMPONENTE_ORGANISATIONEN_CREATED_BY', 'a.created_by', $listDirn, $listOrder); ?>
				</th>
			...
			<?php foreach ($this->items as $i => $item) :
                ...
                <?php if (isset($this->items[0]->state)): ?>
					<td class="center">
						<?php echo JHtml::_('jgrid.published', $item->state, $i, 'organisationen.', $canChange, 'cb'); ?>
					</td>
                <?php endif; ?>
				...
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_einsatzkomponente&task=organisation.edit&id='.(int) $item->id); ?>">
