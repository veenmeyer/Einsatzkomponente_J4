...
if ($saveOrder)
{
	...
	JHtml::_('sortablelist.sortable', 'einsatzartList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
...
?>
<form action="<?php echo JRoute::_('index.php?option=com_einsatzkomponente&view=einsatzarten'); ?>" method="post" name="adminForm" id="adminForm">
...
<?php endif;?>
		...
		<table class="table table-striped" id="einsatzartList">
			<thead>
				...
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_EINSATZKOMPONENTE_EINSATZARTEN_TITLE', 'a.title', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				...
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_EINSATZKOMPONENTE_EINSATZARTEN_ICON', 'a.icon', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_EINSATZKOMPONENTE_EINSATZARTEN_LIST_ICON', 'a.list_icon', $listDirn, $listOrder); ?>
				</th>
			...
			<?php foreach ($this->items as $i => $item) :
                ...
                <?php if (isset($this->items[0]->state)): ?>
					<td class="center">
						<?php echo JHtml::_('jgrid.published', $item->state, $i, 'einsatzarten.', $canChange, 'cb'); ?>
					</td>
                <?php endif; ?>
                    
				<td>
				<?php if (isset($item->checked_out) && $item->checked_out) : ?>
					<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'einsatzarten.', $canCheckin); ?>
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_einsatzkomponente&task=einsatzart.edit&id='.(int) $item->id); ?>">
				...
				?>
				<td>
				<a href="<?php echo JRoute::_('index.php?option=com_einsatzkomponente&view=einsatzberichte&filter_data1='.(int) $item->id); ?>">
