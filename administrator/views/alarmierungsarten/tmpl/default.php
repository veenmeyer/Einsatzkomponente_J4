...
if ($saveOrder)
{
	...
	JHtml::_('sortablelist.sortable', 'alarmierungsartList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
...
?>
<form action="<?php echo JRoute::_('index.php?option=com_einsatzkomponente&view=alarmierungsarten'); ?>" method="post" name="adminForm" id="adminForm">
...
<?php endif;?>
		...
		<table class="table table-striped" id="alarmierungsartList">
			<thead>
                ...
                <?php endif; ?>
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_EINSATZKOMPONENTE_ALARMIERUNGSARTEN_TITLE', 'a.title', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JText::_('COM_EINSATZKOMPONENTE_ANZAHL_EINSAETZE');?>				</th>
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_EINSATZKOMPONENTE_ALARMIERUNGSARTEN_IMAGE', 'a.image', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_EINSATZKOMPONENTE_ALARMIERUNGSARTEN_CREATED_BY', 'a.created_by', $listDirn, $listOrder); ?>
				</th>
			...
			<?php foreach ($this->items as $i => $item) :
                ...
                <?php if (isset($this->items[0]->state)): ?>
					<td class="center">
						<?php echo JHtml::_('jgrid.published', $item->state, $i, 'alarmierungsarten.', $canChange, 'cb'); ?>
					</td>
                <?php endif; ?>
				<td>
				<?php if (isset($item->checked_out) && $item->checked_out) : ?>
					<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'alarmierungsarten.', $canCheckin); ?>
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_einsatzkomponente&task=alarmierungsart.edit&id='.(int) $item->id); ?>">
